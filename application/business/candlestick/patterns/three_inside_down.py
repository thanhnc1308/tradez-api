...
class ThreeInsideDown(CandlestickFinder):
    ...
    def logic(self, idx):
        ...
        return ((b_prev_close>b_prev_open) and (prev_open>prev_close) and (prev_open<= b_prev_close) and (b_prev_open<= prev_close) and ((prev_open-prev_close)<(b_prev_close-b_prev_open)) and (open>close) and (close<prev_close) and (open<prev_open))
