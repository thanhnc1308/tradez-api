...
class BullishHaramiCross(CandlestickFinder):
    ...
    def logic(self, idx):
        ...
        is_bullish_harami_cross_pattern = ((prev_open > open) and (prev_close < open) and (prev_close < close)and (prev_open  > low) and (prev_high  > high))
        is_second_day_doji  = self.approximate_equal(open, close)

        return (is_bullish_harami_cross_pattern and is_second_day_doji)