...
class ThreeInsideUp(CandlestickFinder):
    ...
    def logic(self, idx):
        ...
        return ((b_prev_open>b_prev_close) and (prev_close>prev_open) and (prev_close<= b_prev_open) and (b_prev_close<= prev_open) and ((prev_close-prev_open)<(b_prev_open-b_prev_close)) and (close>open) and (close>prev_close) and (open>prev_open))
